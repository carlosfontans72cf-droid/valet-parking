-- ============================================
-- VALET PARKING - Esquema Completo de BD
-- ============================================
...
-- 4. TABLA: sectores (zonas de estacionamiento)
...
-- 9. TABLA: historial_completo (trazabilidad total)
...
CREATE INDEX idx_tickets_valet_entrada ON tickets(id_valet_entrada);
CREATE INDEX idx_historial_ticket ON historial_completo(id_ticket);
CREATE INDEX idx_historial_evento ON historial_completo(id_evento);
CREATE INDEX idx_historial_fecha ON historial_completo(creado_en DESC);
CREATE INDEX idx_cambios_ticket ON cambios_ubicacion(id_ticket);
...
-- Función para obtener el último número de ticket + 1
...
-- Función para registrar en historial automáticamente
...
BEGIN
  ...
  VALUES (
    NEW.id,
    NEW.id_evento,
    NEW.id_valet_entrada,
    'entrada',
    jsonb_build_object(
      'sector', (SELECT nombre FROM sectores WHERE id = NEW.id_sector),
      'ubicacion', NEW.ubicacion_exacta,
      'llave', NEW.estado_llave
    )
  );
  RETURN NEW;
END;
...
CREATE TRIGGER trigger_registrar_entrada
  AFTER INSERT ON tickets
  FOR EACH ROW
  EXECUTE FUNCTION registrar_historial();

-- Función para incrementar un contador (usada por RPC)
CREATE OR REPLACE FUNCTION increment(x INTEGER)
RETURNS INTEGER
LANGUAGE SQL
AS $$
  SELECT COALESCE((
    SELECT vehiculos_totales + x FROM eventos WHERE id = (SELECT id FROM eventos ORDER BY fecha_apertura DESC LIMIT 1)
  ), x);
$$;

-- Disparador para actualizar vehiculos_totales del evento al crear ticket
...
CREATE TRIGGER trigger_actualizar_conteo
  AFTER INSERT ON tickets
  FOR EACH ROW
  EXECUTE FUNCTION actualizar_conteo_evento();
