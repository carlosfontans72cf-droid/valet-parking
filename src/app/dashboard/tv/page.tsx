    ...
    <div className="min-h-screen bg-gray-950 text-white p-8" style={{ fontFamily: "'Segoe UI',sans-serif" }}>
      ...
      {evs.length > 0 && (
        <div className="flex gap-4 mb-8">
          {evs.map((ev: any) => (
            <button key={ev.id} onClick={() => setEvSel(ev.id)} className={`rounded-3xl px-8 py-4 transition-all cursor-pointer ${evSel === ev.id ? "bg-blue-600 shadow-lg ring-2 ring-blue-300" : "bg-gray-800 hover:bg-gray-700"}`}>
      ...
      <div className="grid grid-cols-2 gap-6" style={{ height: 'calc(100vh - 300px)' }}>
        <div className="flex flex-col">
          <h2 className="text-3xl font-semibold mb-4 flex items-center gap-4">🅿️ Sectores <span className="text-5xl font-bold text-blue-400">{total}</span><span className="text-xl font-normal text-gray-400">/1305</span></h2>
              ...
              return (
                <div key={s.id} className="bg-gray-900 rounded-2xl p-3">
                  <div className="flex items-center justify-between mb-1">
                    <div className="flex items-center gap-2"><span className="w-3 h-3 rounded-full" style={{ backgroundColor: s.color_hex }} /><span className="text-lg font-semibold">{s.nombre}</span></div>
                  ...
                  <div className="w-full bg-gray-800 rounded-full h-3 overflow-hidden"><div className="h-full rounded-full transition-all duration-1000" style={{ width: `${Math.min(pct, 100)}%`, backgroundColor: s.color_hex }} /></div>
                </div>
              );
            })}
          </div>
            ...
            {tkts.map((t: any, i: number) => (
              <div key={i} className="bg-gray-900 rounded-2xl p-4 flex items-center gap-4">
                ...
                <div className="flex-1">
                  <div className="flex items-center gap-3"><span className="w-3 h-3 rounded-full" style={{ backgroundColor: t.sector_color }} /><span className="text-xl font-semibold" style={{ color: t.sector_color }}>{t.sector_nombre}</span></div>
                  <p className="text-sm text-gray-400 mt-1">📍 {t.ubicacion_exacta}</p>
                </div>
              </div>
            ))}
