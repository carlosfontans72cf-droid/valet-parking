  ...
  return (
    <div className="min-h-screen bg-gray-50 p-4" style={{ maxWidth: 640, margin: "0 auto" }}>
      ...
      <div className="grid grid-cols-2 gap-2 mb-6">
        <button onClick={() => window.location.href = "/valet/entrada"} className="bg-gradient-to-br from-blue-600 to-blue-800 text-white rounded-2xl p-5 text-center shadow-lg"><span className="text-3xl block mb-1">🚗</span><span className="font-bold text-lg">Registrar Entrada</span></button>
        <button onClick={() => window.location.href = "/dueno/configuracion"} className="bg-gradient-to-br from-purple-600 to-purple-800 text-white rounded-2xl p-5 text-center shadow-lg"><span className="text-3xl block mb-1">🔑</span><span className="font-bold text-lg">Crear Valet</span></button>
      </div>
        ...
        <div className="space-y-1.5 max-h-96 overflow-y-auto">
          {hist.slice(0, 50).map((h: any) => (
            <div key={h.id} className="flex items-center gap-2 p-2 text-xs border-b border-gray-100">
              <span className="text-lg">{h.tipo === "entrada" ? "🚗" : h.tipo === "retiro_entregado" ? "✅" : h.tipo === "cambio_sector" ? "🔄" : "📍"}</span>
              {h.tn && <span className="font-bold text-gray-700">#{h.tn}</span>}
              <span className="text-gray-500 uppercase font-medium">{h.tipo === "retiro_entregado" ? "ENTREGÓ" : h.tipo === "cambio_sector" ? "CAMBIO" : h.tipo === "entrada" ? "ENTRADA" : h.tipo}</span>
