...
export default function EntregarPage() {
  ...
  const buscar = async () => {
    setErr(""); setT(null);
    if (!n||!eid) { setErr("Falta evento o número"); return; }
    ...
    const [sec, val, secs, hist] = await Promise.all([
      ...
      q(`${SB}/rest/v1/sectores?select=id,nombre,color_hex&activo=eq.true`),
      q(`${SB}/rest/v1/historial_completo?select=id,tipo,id_valet,creado_en,detalles&id_ticket=eq.${tk.id}&order=creado_en.asc`),
    ]);

    const sn = Array.isArray(sec)&&sec.length?sec[0].nombre:"—";
    const sc = Array.isArray(sec)&&sec.length?sec[0].color_hex:"#666";
    const vn = Array.isArray(val)&&val.length?val[0].nombre:"—";
    const sm = Array.isArray(secs)?secs:[];

    // Enriquecer historial con nombres y detalles
    let he: any[] = [];
    if (Array.isArray(hist)) {
      he = await Promise.all(hist.map(async (h: any) => {
        const p = await q(`${SB}/rest/v1/perfiles?select=nombre&id=eq.${h.id_valet}`);
        const pn = Array.isArray(p)&&p.length?p[0].nombre:"—";
        let det = "";
        if (h.detalles && typeof h.detalles === "object") {
          const dd = h.detalles as any;
          if (dd.sector_anterior) det = `${dd.sector_anterior} → ${dd.sector_nuevo}`;
        }
        return { ...h, vn: pn, det, icon: h.tipo==="entrada"?"🚗":h.tipo==="cambio_sector"?"🔄":"✅", label: h.tipo==="entrada"?"INGRESÓ":h.tipo==="cambio_sector"?"REUBICÓ":h.tipo==="retiro_entregado"?"ENTREGÓ":h.tipo.toUpperCase() };
      }));
    }

    setT({ ...tk, sn, sc, vn, sm, hist: he });
  };

  const ent = async () => {
    if (!t||!vid) return;
    try {
      await fetch(`${SB}/rest/v1/tickets?id=eq.${t.id}`, { method:"PATCH", headers:{...H,"Content-Type":"application/json"}, body:JSON.stringify({ estado:"completado", hora_salida:new Date().toISOString(), id_valet_salida:vid }) });
      await fetch(`${SB}/rest/v1/historial_completo`, { method:"POST", headers:{...H,"Content-Type":"application/json"}, body:JSON.stringify({ id_ticket:t.id, id_evento:t.id_evento, id_valet:vid, tipo:"retiro_entregado" }) });
      ...
      <div className="max-w-lg mx-auto space-y-4">
        {!t && (
          <div className="flex gap-2">
            <input value={n} onChange={e=>setN(e.target.value.replace(/\D/g,''))} className="flex-1 p-4 text-3xl bg-gray-800 border border-gray-600 rounded-2xl text-white text-center font-bold" placeholder="N° ticket" />
        ...
        {t && (
          <div className="bg-gray-800 rounded-2xl p-5">
            ...
            <div className="mt-4">
                ...
                <div className="space-y-1">
                  {t.hist.map((m:any,i:number)=>(
                    <div key={i} className="bg-gray-900/70 rounded-lg p-2.5 text-xs flex items-center gap-2">
                      ...
                      <span className="text-gray-400">{m.vn}</span>
              ...
              ) : <p className="text-gray-500 text-xs text-center py-2">Sin movimientos registrados</p>}
            </div>
        ...
        {err&&<div className="bg-red-900/50 text-red-300 p-4 rounded-2xl text-sm text-center">{err}</div>}
        {ok&&<div className="bg-green-900/50 text-green-300 p-4 rounded-2xl text-sm text-center">{ok}</div>}
      </div>
    </div>
  );
}
