...
const p = (u: string) => fetch(u, { headers: { apikey: "sb_publishable_ALyCDA4qM4T68YiecEQErQ_WoYNUfen", Authorization: "Bearer sb_publishable_ALyCDA4qM4T68YiecEQErQ_WoYNUfen" } }).then(r => r.json()).catch(() => null);
...
export default function EntradaPage() {
  ...
  useEffect(() => {
    p(SB + "sectores?select=id,nombre,capacidad,color_hex&activo=eq.true&order=orden").then(async (d) => {
      if (Array.isArray(d)) {
        const w = await Promise.all(d.map(async (s: any) => {
          const t = await p(SB + "tickets?select=id&id_sector=eq." + s.id + "&estado=eq.activo");
          return { ...s, activos: Array.isArray(t) ? t.length : 0 };
        }));
        setSecs(w);
      }
    });
    if (evId) p(SB + "tickets?select=numero_ticket&id_evento=eq." + evId + "&order=numero_ticket.desc&limit=1").then(d => setNumT(String((Array.isArray(d) && d.length ? d[0].numero_ticket : 0) + 1)));
  }, []);
  ...
  const reg = async () => {
    ...
    const ex = await p(SB + "tickets?select=id&numero_ticket=eq." + n + "&id_evento=eq." + evId + "&estado=eq.activo");
    if (Array.isArray(ex) && ex.length) { setErr("Ticket #" + n + " ya activo"); return; }
    setBusy(true);
    try {
      const patStr = pat.trim() ? pat.toUpperCase().trim() : "T-" + n;
      let idV = "";
      const vd = await p(SB + "vehiculos?patente=eq." + encodeURIComponent(patStr));
      if (Array.isArray(vd) && vd.length) idV = vd[0].id;
      else {
        const r = await fetch(SB + "vehiculos", { method: "POST", headers: { "Content-Type": "application/json", apikey: "sb_publishable_ALyCDA4qM4T68YiecEQErQ_WoYNUfen", Authorization: "Bearer sb_publishable_ALyCDA4qM4T68YiecEQErQ_WoYNUfen", Prefer: "return=representation" }, body: JSON.stringify({ patente: patStr, modelo: mod, color: col }) });
        const d = await r.json();
        idV = Array.isArray(d) && d.length ? d[0].id : d?.id;
      }
      if (!idV) { setErr("Error al crear vehículo"); setBusy(false); return; }
      await fetch(SB + "tickets", { method: "POST", headers: { "Content-Type": "application/json", apikey: "sb_publishable_ALyCDA4qM4T68YiecEQErQ_WoYNUfen", Authorization: "Bearer sb_publishable_ALyCDA4qM4T68YiecEQErQ_WoYNUfen", Prefer: "return=representation" }, body: JSON.stringify({ numero_ticket: n, id_evento: evId, id_vehiculo: idV, id_sector: sSel, ubicacion_exacta: ubi || "—", estado_llave: llave, id_valet_entrada: vId, estado: "activo", ticket_cliente_entregado: tCli, ticket_auto_colocado: tAuto }) });
      setOk("🎫 #" + n + " registrado!");
